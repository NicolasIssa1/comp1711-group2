...
int main()
{
    ...
    {   
        ...
        tokeniseRceord(line,",",pos,artist,title,year);
        // but then what??

        strcpy(songs[counter].artist,artist);
/workspaces/comp1711-group2/week_4/session2        strcpy(songs[counter].title,title);
        strcpy(songs[counter].year,year);
        songs[counter].position = atoi(pos);
        counter++;
    }
    // Count how many totals songs you read in
    printf("Total songs read : %d\n",counter);
    fclose(input);
    return 0;
